  ...
  return (serviceName, service) => {
    ...
    if (!service.disabled) {
      ...
      let build = {
        ...
        model: service.model ? util.anyExists(sourcePaths, sourceNames, `${modelBasePath}${service.model}.js`) : { path: false, source: sourceNames[2]},
        service: !service.auto ? util.anyExists(sourcePaths, sourceNames, `${serviceBasePath}${serviceName}`) : { path: false, source: sourceNames[2]}
      }
      ...
      console.log(`Mounting /${build.service.mount} (${build.service.source})`,
      ...
      if (service.auto) {
        ...
        if (!build.service.path) {
          console.log(`\nERROR: The folder path for this service does not exist.\nCheck the path and fix it in order to mount the service: "${util.root()}${serviceBasePath}${serviceName}"`);
        } else {
          try {
      ...
      try {
        ...
        console.log(`\nERROR: The service /${build.service.mount} was not mounted. This is usually because the folder path or name is not what you expected.\n`)
      }
    }
    return Model;
  };
